...
public class JobServiceImpl {
    ...
    public List<Integer> getScheduledJobs() {
        List<JobBean> jobList = this.jobDao.getAutoRunJobs();
        List<Integer> rs = new ArrayList<Integer>();
        for (JobBean job : jobList) {
            if (EJobRunType.AUTO.ordinal() == job.getAuto()) {
                rs.add(job.getId());
            }
        }
    ...
    public List<JobBean> findJobByContent(String key) {
        return jobDao.findByScript(key);
    }
}
